...
- `piece_store.py` / `piece_cache.py`: disk reads and the byte-budgeted in-RAM cache (bulk-warm on multi-token passes; single-token decode warm is an opt-in engine flag, measured no-effect). `sysmem.py` provides the auto budget — **0.75 × RAM − 2 GiB** (16 GB → 10) — and `wire_memory_limit()`, which the engine calls by default (`wire_limit=True`, CLI `--no-wire`) to wire the MLX buffer pool to the device's recommended working-set size. The old "more budget is not faster" cliff was a WIRING artifact (backlog #14): unwired, the macOS compressor stole the cache; wired, throughput rises monotonically with budget as long as budget + ~1 GB overhead stays under the wire (measured 16 GB wired sweep: 6/8/9/10/11 GB = 4.6/6.4/6.7/7.8/6.4 tok/s — 11 GB crosses the 11.84 GB wire and regresses). An availability-based clamp was tried and reverted, don't reintroduce it.
